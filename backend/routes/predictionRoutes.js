...
// Helper function to ensure models are trained if the primary one isn't.
async function ensureModelsTrained(res) {
    // 1. Check current status of the baseline model
    const linearRegressionInfo = modelManager.getModelInfo('linear-regression');
    
    // 2. If LR is not trained, attempt a full retraining of all models
    if (!linearRegressionInfo.trained) {
        const allData = await FoodData.find();
        ...
        // 3. Log any models that failed training (e.g., KNN/RF due to low data)
        Object.entries(trainResults).forEach(([name, result]) => {
            if (!result.success) {
                console.warn(`[ML Training Warning]: Model ${name} failed training: ${result.message}`);
                // NOTE: We do not return a 400 error here unless LR failed, 
                // allowing low-data models to fail gracefully.
            }
        });
        
        // 4. Critical check: If the core Linear Regression model still failed, stop.
        if (!trainResults['linear-regression'].success) {
            return res.status(400).json({ 
                message: 'Initial model training failed due to core linear regression errors.',
                details: trainResults['linear-regression']
            });
        }
    }
    // If LR was already trained, or successfully trained now, proceed.
    return null; 
}
...
router.post('/', async (req, res) => {
    try {
        ...
        const modelNames = modelManager.getAvailableModels(); 
        ...
        for (const modelName of modelNames) {
            try {
                // Ensure planned_batch_kg is present and valid before calling predict
                if (!inputData.planned_batch_kg || inputData.planned_batch_kg <= 0) {
                    throw new Error('Planned batch (kg) must be provided and greater than zero.');
                }
                
                // Only attempt prediction if the model is currently trained
                if (modelManager.getModelInfo(modelName).trained) {
                     const result = modelManager.predict(modelName, inputData);
                     allModelPredictions[modelName] = result;
                } else {
                     allModelPredictions[modelName] = { 
                        error: true, 
                        message: `${modelName} skipped prediction: Not enough data to train.` 
                    };
                }

            } catch (err) {
                // Catch internal prediction errors (e.g., division by zero, unexpected model state)
                allModelPredictions[modelName] = { 
                    error: true, 
                    message: `Prediction failed for ${modelName}: ${err.message}` 
                };
        ...
        // Only save if the primary LR model provided a non-error result
        ...
        res.json({
            message: 'Predictions generated by all available models.',
            prediction_id: prediction._id || 'N/A', // Send ID only if saved
            input_data: inputData,
            results: allModelPredictions // All results returned here
        });
    } catch (error) {
...
// Batch prediction for multiple items (Updated to use getAvailableModels)
router.post('/batch', async (req, res) => {
    try {
        const items = req.body.items;
        ...
        // Ensure models are trained before batch prediction
        const trainingErrorResponse = await ensureModelsTrained(res);
        if (trainingErrorResponse) return trainingErrorResponse;

        // Check if the requested model is actually trained
        if (!modelManager.getModelInfo(modelName).trained) {
             return res.status(400).json({ message: `Model ${modelName} is not trained and cannot perform batch prediction.` });
        }


        const predictions = [];
        for (const item of items) {
             if (!item.planned_batch_kg || item.planned_batch_kg <= 0) {
                 predictions.push({
                     dish_name: item.dish_name || 'Unknown Dish',
                     error: true,
                     message: 'Missing or invalid planned_batch_kg for prediction.'
                 });
                 continue;
            }
...
// NEW: Endpoint to manually trigger full retraining of all models (No change needed)
router.post('/retrain', async (req, res) => {
    try {
        const allData = await FoodData.find();
        if (allData.length === 0) {
